...
namespace Indigo.Server.Controllers
{
    ...
    public class ConversationsApiController : Controller
    {
        ...
        // PUT: api/conversations/5/users/5
        [HttpPut("{conversationid}/users/{userid}")]
        public async Task<IActionResult> PutUserConversationAsync([FromHeader] string authUsername, [FromHeader] string authPasswordHash,
            [FromRoute] int conversationid, [FromRoute] int userid, [FromBody] UserConversation userConversation)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (conversationid != userConversation.ConversationId && userid == userConversation.UserId)
            {
                return BadRequest();
            }
